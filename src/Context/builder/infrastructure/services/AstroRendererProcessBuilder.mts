...
export class AstroRendererProcessBuilder implements RendererProcessBuilderService {
  ...
  public async build(outputDirectory: string, config: RendererConfig): Promise<void> {
    try {
      ...
      const options = await this.loadRendererAstroOptions(outputDirectory, config);
  ...
  public async develop(outputDirectory: string, config: RendererConfig, preloadEntryPoints: string[]): Promise<void> {
    try {
      ...
      const options = await this.loadRendererAstroDevOptions(outputDirectory, config);
      const server = await dev(options);
  ...
  public async loadRendererAstroDevOptions(output: string, config: RendererConfig): Promise<AstroInlineConfig> {
    const baseConfig = await this.loadRendererAstroOptions(output, config);
    const devServerPort = await findFreePort(config.devPort, true);
